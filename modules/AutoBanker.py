...
from engine.Scanners.ScanCap import ScanCap
    ...
    def __init__(self, root):
        ...
        def ScanAutoBanker():
            if EnabledAutoBanker:
                CapLocate = pyautogui.locateOnScreen(
                    'images/PlayerStats/Cap.png')
                if CapLocate:
                    FirstNumberBox = CapLocate[0] + 21, CapLocate[1] + 7
                    EndFirstNumberBox = FirstNumberBox[0] + \
                        7, FirstNumberBox[1] + 10

                    SecondNumberBox = CapLocate[0] + 15, CapLocate[1] + 7
                    EndSecondNumberBox = SecondNumberBox[0] + \
                        7, SecondNumberBox[1] + 10

                    ThirdNumberBox = CapLocate[0] + 9, CapLocate[1] + 7
                    EndThirdNumberBox = ThirdNumberBox[0] + \
                        7, ThirdNumberBox[1] + 10

                    FourNumberBox = CapLocate[0] + 3, CapLocate[1] + 7
                    EndFourNumberBox = FourNumberBox[0] + \
                        7, FourNumberBox[1] + 10
        ...
        self.AutoBanker.addButton('Ok', self.AutoBanker.destroyWindow, [
                                  84, 29, 130, 504], [127, 17, 8], [123, 13, 5])
        ...
        ButtonPrint = self.AutoBanker.addCheck(
            CheckPrint, [10, 408], [120, 98, 51], 0, "Print on Tibia's screen")

        ButtonLowMana = self.AutoBanker.addCheck(
            LowMana, [10, 440], [120, 98, 51], 0, "Low Mana Warnings")
